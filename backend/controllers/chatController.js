...
const createGroupChat = asyncHandler(async (req,res)=>{
    ...
    if(users.length < 2){
        return res.status(400).send("More than 2 users are required to foam a group chat");
    }
    users.push(req.user);
    try{
...
const removeFromGroup = asyncHandler(async (req,res)=>{
    const {chatId, userId} = req.body;
    const removeUser = await Chat.findByIdAndUpdate(chatId, {$pull:{users:userId}}, {new:true}).populate("users","-password").populate("groupAdmin", "-password")
    if(!removeUser){
        res.status(404);
        throw new Error("Enable to remove user");
    }
    else{
        res.json(removeUser);
    }
